...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...

  {
    ...
    \begin{frame}{Summary}
      Benefits:
      \begin{itemize}[<+- | alert@+>]
        \item Capsules introduce a new building block that can be used in deep learning to better model hierarchical relationships
        \item representations similar to scene graphs in computer graphics
        \item there was no algorithm that allowed to implement and successfully learn a capsule network
        \item One of the reasons is that computers were just not powerful enough in the pre-GPU-based era before around 2012
        \item Another benefit of the capsule approach is that it is capable of learning to achieve state-of-the art performance by only using a fraction of the data that a CNN would use
        \item to learn to tell digits apart, the human brain needs to see only a couple of dozens of examples, hundreds at most. CNNs, on the other hand, need tens of thousands of examples
      \end{itemize}
      \pause

      Downsides:
      \begin{itemize}[<+- | alert@+>]
        \item Current implementations are much slower than other modern deep learning models.
      \end{itemize}
    \end{frame}
  }
